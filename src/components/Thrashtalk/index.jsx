...
function Thrashtalk({ firebase }) {
  ...
  const { uid, metadata } = firebase.auth.currentUser;

  const lastSignInTime = metadata?.lastSignInTime;

  // faker.locale = "sv";
  // faker.seed(hashCode(uid + moment().format("DD/MM") + lastSignInTime));
